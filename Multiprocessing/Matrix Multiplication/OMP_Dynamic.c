...
void matrix_multiplication(double** matrixA, double** matrixB, int rowsA, int colsA, int colsB, double** result) { 
    #pragma omp parallel for num_threads(colsB)
    ...
    #pragma omp parallel for num_threads(colsB) collapse(2)
    for (int i = 0; i < rowsA; i++) {
            for (int j = 0; j < colsB; j++) {
	        double sum = 0.0;
...
int main(int argc, char *argv[]) {
    ...
    int cpu_time_used = ((end - start)*1000) / CLOCKS_PER_SEC;
    printf("%d", cpu_time_used);

    // Save the result to a file
    save_matrix_to_file(result, rowsA, colsB, "matrixC_OMP.txt");
